...
const range = (days) => {
  ...
  return { from: from.toISOString().slice(0, 19).replace("T", " "),
    to: to.toISOString().slice(0, 19).replace("T", " "), days: n };
};
...
function overview(storeId, days) {
  ...
  const prevFrom = new Date(Date.parse(from + "Z") - n * 86400000)
...
function busy(storeId, days) {
  ...
  db.prepare("SELECT at, total FROM sales WHERE store_id = ? AND at >= ? AND at <= ? " +
    ...
    const d = new Date(String(r.at).replace(" ", "T") + (String(r.at).endsWith("Z") ? "" : "Z"));
    if (isNaN(d)) return;
...
function exceptions(storeId, days) {
  ...
  walk(storeId, from, to, (r, s) => {
    ...
    const h = new Date(String(r.at).replace(" ", "T") + "Z").getUTCHours();
    if (h >= 2 && h < 5)
...
function day(storeId, date, deptNames) {
  ...
  const depts = {};
  walk(storeId, from, to, (r, s) => {
    const d = new Date(String(r.at).replace(" ", "T") + "Z");
