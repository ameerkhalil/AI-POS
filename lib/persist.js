...
/* Contributed after setup and again as the store settles, so the shape other
   stores learn from is the one actually in use rather than the one guessed at. */
/* The retention choice is a server-side policy, not a config field — the wipe
   has to happen whether or not anyone opens the terminal. */
async function saveRetention() {
  if (!STORE_ID || typeof A === "undefined" || !ONLINE) return;
  try {
    await api("/api/retention?store=" + STORE_ID, { method: "PUT",
      body: { store: STORE_ID, mode: A.keep === "wipe" ? "wipe" : "keep",
        days: parseInt(A.keepDays) || 90, email: A.keepEmail || "", exportFirst: true } });
  } catch (e) {}
}

async function contributePattern() {
  if (!STORE_ID || !CFG || !ONLINE) return;
  try {
    await api("/api/learn/contribute?store=" + STORE_ID, { method: "POST",
      body: { store: STORE_ID, bizType: CFG.bizType || (typeof A !== "undefined" ? A.type : ""), config: CFG } });
  } catch (e) {}
}
...
async function initApp() {
  ...
  if (me) {
    ...
    if (!pick) {
      /* An operator runs the service rather than a shop, so it owns no store.
         Sending it back to login was a loop: log in, no store, log in again. */
      if (me.operator) { location.href = "/admin.html"; return; }
      /* A merchant with no store needs one made, not a bounce to the sign-in
         page they just came from. */
      const name = prompt("What's this store called?", "My store");
      if (!name || !name.trim()) { location.href = "/login.html"; return; }
      try {
        const d = await api("/api/stores", { method: "POST", body: { name: name.trim() } });
  ...
  if (cfg) {
    ...
    ensureTerminal().then(startHeartbeat);
    if (typeof displayInit === "function") displayInit();
    if (typeof startErrorReporting === "function") startErrorReporting();
    ...
    /* An operator who ends up here — because they were given a store, or the
       flag landed after the store did — should not have to complete a shop
       setup to reach the console. */
    if (me && me.operator) operatorEscape();
  }
}

/* A standing way out of the wizard for whoever runs the service. */
function operatorEscape() {
  if (document.getElementById("opEscape")) return;
  const bar = document.createElement("div");
  bar.id = "opEscape";
  bar.className = "opescape";
  bar.innerHTML = `<span>You're signed in as an operator of this service, not a merchant.</span>
    <a href="/admin.html">Open the fleet console</a>`;
  document.getElementById("setup").prepend(bar);
}

/* ------------------------------- this terminal ----------------------------
   A register identifies itself with a token kept on the device. It says which
   lane this is — the session still proves who you are, so the token on its own
   opens nothing. */
let TERMINAL = null, TERM_ID = null;

async function ensureTerminal() {
  const key = "pos_terminal_" + STORE_ID;
  TERMINAL = localStorage.getItem(key);
  if (TERMINAL) return TERMINAL;
  try {
    const t = await api("/api/terminals?store=" + STORE_ID, { method: "POST",
      body: { store: STORE_ID, name: "" } });
    localStorage.setItem(key, t.token);
    TERMINAL = t.token;
    TERM_ID = t.id;
    if (CFG && CFG.site) { CFG.site.register = String(t.number); queueSave(); }
  } catch (e) { /* offline: it registers on the next connection */ }
  return TERMINAL;
}

/* Frequent and cheap. Doubles as the fleet's "is that register alive" signal. */
function startHeartbeat() {
  const beat = async () => {
    if (!TERMINAL || !ONLINE) return;
    try {
      const hb = await api("/api/heartbeat?store=" + STORE_ID, { method: "POST",
        body: { store: STORE_ID, token: TERMINAL,
          build: typeof BUILD !== "undefined" ? BUILD : null,
          agent: typeof STATION !== "undefined" && !!STATION,
          queued: typeof queuedCount === "function" ? await queuedCount() : 0 } });
      if (hb && hb.id) TERM_ID = hb.id;
    } catch (e) {}
  };
  beat();
...
function accountMenu() {
  api("/api/me").then(me => {
    ...
    /* This used to open a dialog with no text field and then hunt for one,
       which meant the Create button did nothing at all. A dialog that needs a
       name should be built with a name box in it. */
    el.querySelector("#newStore").onclick = () => {
      el.remove();
      const box = veil(`<div class="card"><h3>Add a store</h3>
        <p>Each store keeps its own pricebook, staff, shifts and reports. You can move between
          them from this menu at any time.</p>
        <input id="nsName" placeholder="Second location" autocomplete="off"
          style="margin-top:15px;font-size:16px">
        <div id="nsMsg" class="note" style="margin-top:9px;display:none"></div>
        <div class="row"><button class="no" id="nsx">Cancel</button>
          <button class="ok" id="nsy">Create it</button></div></div>`);

      const inp = box.querySelector("#nsName");
      const msg = box.querySelector("#nsMsg");
      const go = box.querySelector("#nsy");
      const fail = t => { msg.textContent = t; msg.style.display = "block"; };

      const create = async () => {
        const name = inp.value.trim();
        if (!name) return fail("Give the store a name.");
        go.disabled = true;
        go.textContent = "Creating…";
        try {
          const d = await api("/api/stores", { method: "POST", body: { name } });
          /* Switching to it immediately is the point — a store you have to go
             and find afterwards feels like nothing happened. */
          localStorage.setItem("pos_store", String(d.id));
          location.reload();
        } catch (e) {
          go.disabled = false;
          go.textContent = "Create it";
          fail(e.message || "Couldn't create that store.");
        }
      };

      go.onclick = create;
      inp.onkeydown = e => { if (e.key === "Enter") create(); };
      box.querySelector("#nsx").onclick = () => box.remove();
