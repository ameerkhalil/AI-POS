...
/* --------------------------------- who is it -------------------------------
   A time clock anybody can press is worthless — punching a colleague in is the
   oldest payroll fraud there is. So the person's own code is required, and it's
   checked HERE rather than in the browser: a till is a machine in a shop, and
   the code that decides whether a wage claim is valid shouldn't live on it.

   The codes sit in the store's own configuration, which the server already
   holds, so this costs one read. */
const attempts = new Map();
const TRIES = 5, LOCKOUT = 60000;

function staffOf(storeId) {
  const row = db.prepare("SELECT json FROM configs WHERE store_id = ?").get(storeId);
  if (!row) return [];
  try { return JSON.parse(row.json || "{}").employees || []; } catch (e) { return []; }
}

/* Returns the person the code belongs to, or null. Never says which part was
   wrong — "no such person" and "wrong code" are the same answer, or the
   failures themselves become a way to learn the codes. */
function whoseCode(storeId, code) {
  const pin = String(code || "").trim();
  if (!pin) return null;
  return staffOf(storeId).find(e => String(e.pin || "") === pin) || null;
}

function tooManyTries(storeId, who) {
  const key = `${storeId}|${who}`;
  const e = attempts.get(key);
  if (!e) return false;
  if (Date.now() - e.at > LOCKOUT) { attempts.delete(key); return false; }
  return e.n >= TRIES;
}

function noteTry(storeId, who, ok) {
  const key = `${storeId}|${who}`;
  if (ok) { attempts.delete(key); return; }
  const e = attempts.get(key);
  if (!e || Date.now() - e.at > LOCKOUT) attempts.set(key, { at: Date.now(), n: 1 });
  else { e.n++; e.at = Date.now(); }
}

/* Who may punch for whom. Normally only yourself. Somebody with staff
   permission can punch for another person — a manager fixing a forgotten
   punch is a real need — and it's recorded as theirs rather than hidden. */
function mayPunch(storeId, target, code) {
  if (tooManyTries(storeId, target))
    return { ok: false, error: "Too many wrong codes. Wait a minute and try again." };

  const person = whoseCode(storeId, code);
  if (!person) {
    noteTry(storeId, target, false);
    return { ok: false, error: "That code doesn't match." };
  }
  noteTry(storeId, target, true);

  if (person.n === target) return { ok: true, by: person.n, self: true };

  /* Not themselves — allowed only for a role that can manage staff. */
  const role = String(person.role || "").toLowerCase();
  if (role === "manager" || role === "owner" || person.canStaff)
    return { ok: true, by: person.n, self: false };

  return { ok: false,
    error: `That's ${person.n}'s code. Use your own, or ask a manager.` };
}
...
function clockIn(storeId, who, at, code) {
  const allowed = mayPunch(storeId, who, code);
  ...
  const r = db.prepare("INSERT INTO punches (store_id, who, in_at, note) VALUES (?,?,?,?)")
    .run(storeId, String(who), when,
      allowed.self ? null : `clocked in by ${allowed.by}`);
  return { ok: true, id: r.lastInsertRowid, at: when,
    by: allowed.by, onBehalf: !allowed.self };
}

function clockOut(storeId, who, at, note, code) {
  const allowed = mayPunch(storeId, who, code);
  if (!allowed.ok) return { ok: false, error: allowed.error };
  const s = settings(storeId);
  ...
  const stamp2 = allowed.self ? note
    : [note, `clocked out by ${allowed.by}`].filter(Boolean).join(" · ");
  db.prepare("UPDATE punches SET out_at = ?, note = COALESCE(?, note) WHERE id = ?")
    .run(when, stamp2 || null, p.id);
  return { ok: true, id: p.id, by: allowed.by, onBehalf: !allowed.self,
    ...hoursFor({ ...p, out_at: when }, s) };
}
...
/* What one person needs to see at the moment they punch: are they on, how long
   today, how long this week. Deliberately small — this is called on a till in
   front of a customer, not from an office. */
function forPerson(storeId, who) {
  const s = settings(storeId);
  const open = openPunch(storeId, who);
  const today = new Date().toISOString().slice(0, 10);
  const week = weekStart(today);

  const sum = (from, to) => db.prepare(
    "SELECT * FROM punches WHERE store_id = ? AND who = ? AND in_at >= ? AND in_at <= ?")
    .all(storeId, String(who), from + " 00:00:00", to + " 23:59:59")
    .reduce((a, p) => a + (hoursFor(p, s).paidHours || 0), 0);

  const todayHours = sum(today, today);
  const weekHours = sum(week, today);

  /* Days actually worked this week, which is the figure somebody recognises
     better than a decimal. */
  const days = new Set(db.prepare(
    "SELECT in_at FROM punches WHERE store_id = ? AND who = ? AND in_at >= ? AND out_at IS NOT NULL")
    .all(storeId, String(who), week + " 00:00:00")
    .map(p => String(p.in_at).slice(0, 10))).size;

  return {
    who,
    on: !!open,
    since: open ? open.in_at : null,
    /* Minutes on the clock right now, for somebody mid-shift. */
    minutes: open ? Math.round((Date.now() - asDate(open.in_at)) / 60000) : 0,
    todayHours: +todayHours.toFixed(2),
    weekHours: +weekHours.toFixed(2),
    daysThisWeek: days,
    weekStart: week,
    overtimeAfter: s.week_hours,
    /* Worth knowing before they start a shift that tips them over. */
    intoOvertime: weekHours > s.week_hours
  };
}
...
module.exports = { settings, setSettings, wages, setWage, forPerson,
  whoseCode, mayPunch,
