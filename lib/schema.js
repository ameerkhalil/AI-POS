...
const EXPECTED = {
  ...
  services:          ["id", "store_id", "kind", "at", "shift_id", "face", "fee", "cash",
                      "reference", "voided"],
  service_settings:  ["store_id", "money_orders", "mo_fee", "mo_max", "lottery",
                      "lotto_max", "bill_pay", "bp_fee", "billers"],

  error_groups:      ["id", "fingerprint", "kind", "message", "where_", "count",
                      "first_at", "last_at", "terminals", "builds", "resolved"],
  error_events:      ["id", "group_id", "at", "store_id", "terminal", "build", "detail"],

  punches:           ["id", "store_id", "who", "in_at", "out_at", "break_mins",
                      "edited_by", "original"],
  wages:             ["store_id", "who", "rate"],
  clock_settings:    ["store_id", "on_", "week_hours", "ot_multiplier", "round_mins"],

  store_groups:      ["id", "account_id", "name"],
  group_stores:      ["group_id", "store_id"],
  master_items:      ["id", "account_id", "upc", "name", "price", "cost", "dept", "updated_at"],
  price_overrides:   ["store_id", "upc", "field", "value", "reason"],
  pushes:            ["id", "account_id", "group_id", "who", "at", "stores", "changes"],
  scheduled_changes: ["id", "account_id", "upc", "field", "value", "effective", "applied_at"]
};
...
const OWNER = {
  ...
  resets: "server.js",
  schema_migrations: "lib/schema.js",
  services: "lib/services.js", service_settings: "lib/services.js",
  error_groups: "lib/errors.js", error_events: "lib/errors.js",
  punches: "lib/timeclock.js", wages: "lib/timeclock.js",
  clock_settings: "lib/timeclock.js",
  store_groups: "lib/central.js", group_stores: "lib/central.js",
  master_items: "lib/central.js", price_overrides: "lib/central.js",
  pushes: "lib/central.js", scheduled_changes: "lib/central.js"
};
...
const MIGRATIONS = [
  ...
  {
    version: 4,
    name: "normalise timestamps that were stored with a T",
    up: () => {
      /* Sales written before this carry the browser's ISO format. SQLite
         compares dates as strings, so "2026-09-10T04:11:05.855Z" never matched
         a range built as "2026-09-10 04:11:05" — every report came back empty.
         Rewrite them into the one format everything else uses. */
      let n = 0;
      const fix = (table, col) => {
        const rows = db.prepare(
          `SELECT rowid AS rid, ${col} AS v FROM ${table} WHERE ${col} LIKE '%T%'`).all();
        const up = db.prepare(`UPDATE ${table} SET ${col} = ? WHERE rowid = ?`);
        rows.forEach(r => {
          const d = new Date(r.v);
          if (isNaN(d)) return;
          up.run(stamp(d), r.rid);
          n++;
        });
      };
      try { fix("sales", "at"); } catch (e) {}
      try { fix("punches", "in_at"); fix("punches", "out_at"); } catch (e) {}
      try { fix("tobacco_lines", "at"); } catch (e) {}
      return n ? `${n} row${n === 1 ? "" : "s"}` : null;
    }
  }
];
